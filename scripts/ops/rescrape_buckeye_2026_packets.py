#!/usr/bin/env python3
"""
Rescrape all 2026 Buckeye meetings via Granicus pipeline (packet pass only).

Background:
  The existing 65 Buckeye 2026 meetings were synced via the old NovusAgenda
  scraper (buckeye.py), which extracts items from HTML and captures zero
  supporting documents. This script re-runs them through the Granicus pipeline
  (buckeye_granicus.py) which:
...
Usage:
  nohup python3 -u scripts/ops/rescrape_buckeye_2026_packets.py \
    > data/rescrape-buckeye-2026-$(date +%Y%m%d-%H%M).log 2>&1 &
"""
