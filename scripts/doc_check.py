#!/usr/bin/env python3
"""
Doc Availability Checker — lightweight probe for Tempe OnBase meetings.
...
Design:
  - Only checks Tempe OnBase bodies (the only platform with item-level docs
    that get published post-sync).
  - Only checks meetings where next_doc_check_at <= now.
  - Lightweight: one HTTP GET per meeting (the agenda page).
  - Checks for presence of loadAgendaItem(NNN) onclick handlers — if present
    and the meeting is past, we trigger a targeted re-sync.
  - Exponential backoff (+2d → +4d → +8d) if docs aren't available yet.
  - Sunset after 30 days past meeting date: set next_doc_check_at = NULL.
"""
