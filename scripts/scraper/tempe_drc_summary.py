...
meetings only (not Study Sessions or joint meetings).  Unlike the City
Council's Legal Action Summary (which contains roll-call blocks with
individual member names), the DRC summary documents contain per-item
result lines with optional inline vote tallies.

Example format::

    4A.
    Request a Use Permit to ...
    APPROVED ON CONSENT
    ...
    5A.
    Request a Zoning Map Amendment ...
    RECOMMENDED APPROVAL (6-0 VOTE)

    6A.
    Request Zoning and Development Code ...
    RECOMMENDED APPROVAL ON CONSENT

    5A.
    Request a Planned Area Development ...
    CONTINUED TO 6/9/26 DRC MEETING (6-0 VOTE)

    6A.
    Request a Zoning Code Text Amendment ...
    RECOMMENDED APPROVAL ON CONSENT (5-0 VOTE, 1 ABSTAIN)

No individual commissioner names are listed — only aggregate tallies.
"""
...
def fetch_drc_summary_text(meeting_id: int) -> str:
    ...
    # Check for error/page-not-found indicators
...
def _parse_vote_tally(text: str) -> dict:
    ...
    Returns dict with keys ``aye``, ``nay``, ``abstain`` (int, default 0).
    """
...
def extract_items_from_summary(
    ...
    """Parse DRC/BOA/HA summary text and extract agenda item results with vote tallies.

    The summary document format is:

        <number>.
        <description/Title line(s)>
        <RESULT> [optional vote tally on same line]
    ...
    header_prefixes : set[str], optional
        Header line prefixes to skip at the top of the document.
        Defaults to DRC-specific prefixes.
    section_headings : set[str], optional
        Section heading lines to skip entirely.
        Defaults to DRC-specific headings.

    Returns a list of dicts:
        {
            "agenda_item_number": str,   # e.g. "4A", "5A"
            "motion_result": str,        # normalized: "approved", "recommended approval", etc.
            "vote_text": str,            # full result text including tally
            "aye": int,
            "nay": int,
            "abstain": int,
        }
    ...
    if header_prefixes is None:
        header_prefixes = {
            ...
            "Members of the Development Review",
            "Visit",
    ...
    for i, line in enumerate(lines):
        stripped = line.strip()
        if not stripped:
            # Blank line — finalize any pending item
        ...
        # Check for item number patterns like "4A.", "2B.", "1.", "5."
        ...
        # Check for section headings (all-caps without a number) — skip them
        # as they are not individual agenda items.
        if current_item is None and re.match(r"^[A-Z][A-Z\s/]+:$", stripped):
            continue
        if current_item is None and stripped in section_headings:
            continue

        # If we have a current item, check if this line is a result keyword
        # or belongs to the description
        if current_item:
            # Check if this line (possibly combined with a vote tally on same line)
            # is one of the known result keywords
            ...
            for kw in DRC_RESULT_KEYWORDS:
                if result_candidate.startswith(kw):
                    is_result = True
                    break
                # Also check for result + vote tally on same line:
                # e.g. "APPROVED (7-0 VOTE)" or "RECOMMENDED APPROVAL (6-0 VOTE)"
            ...
            if is_result:
                ...
                # Extract the action keyword (first word or two)
                result_text = stripped
                # Normalize: get first meaningful word
                first_part = re.split(r"\s*[(\[]", result_text)[0].strip()
                items.append({
            ...
            # Line before any item — probably header residue
            pass

    # Finalize last item if still open
    if current_item and collected_description:
        current_item["description"] = "\n".join(collected_description).strip()
...
def extract_drc_votes(meeting_id: int) -> dict:
    ...
    Returns a dict with keys:
        votes: list of dicts with agenda_item_number, motion_result,
               vote_text, aye, nay, abstain
    """
...
def backfill_drc_votes(dry_run: bool = True, limit: int = 0,
    ...
    Queries all DRC Regular meetings that have been fully synced
    (sync_status='complete') but have no vote records.  Attempts to
    fetch and parse their summary documents from OnBase.
    ...
    # Find meetings that have NO vote records
    meetings_with_votes = (
    ...
    # DRC meeting types that have summaries: Regular meetings
    ...
    # Filter to only past meetings that likely have summaries published
            ...
            if not dry_run:
                ...
                try:
                    persist_votes(
                        ...
                        [],  # no individual supervisors for DRC
                        vote_data["votes"],
                    )
                    session.commit()
                except Exception as e:
