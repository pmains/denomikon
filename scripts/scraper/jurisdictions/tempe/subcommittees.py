...
See the original source at scripts/scraper/jurisdictions/tempe_subcommittees.py
...
_HELPER_PATH = _PROJECT_ROOT / "scripts" / "scraper" / "platforms" / "tempe_subcommittees_helper.mjs"
...
def _build_meeting_id(body_slug: str, date_str: str, title: str,
                      existing_ids: set[str] | None = None) -> str:
...
def _extract_agenda_items_from_pdf(pdf_url: str, meeting_date: str) -> list[dict] | None:
    """Download a PDF, extract text via pdftotext, and parse agenda items."""
    import tempfile
        ...
        if m:
            ...
            if not item_type:
                for j in range(i + 1, min(i + 5, len(lines))):
                    ...
                    break
            sort_order += 1
            is_substantive = title.lower() not in ("call to order", "adjourn")
            items.append({
...
def sync_subcommittee(body_slug: str, download: bool = False,
                       limit: int | None = None) -> list[dict]:
    """Sync a single subcommittee's agenda documents. Returns meeting dicts created."""
    ...
    if not agenda_docs:
        print("  No agenda documents found.")
        session.close()
        return []
    ...
    for idx, doc in enumerate(agenda_docs):
        ...
        if body_slug not in existing_ids_by_body:
            ...
            rows = session.execute(
                select(MeetingModel.meeting_id).where(MeetingModel.body == body_slug)
            ).fetchall()
        ...
        existing = session.execute(
            select(MeetingModel).where(
                MeetingModel.body == body_slug, MeetingModel.meeting_id == meeting_id,
            )
        ).scalar_one_or_none()
        ...
        meeting_dict = {"meeting_id": meeting_id, "meeting_date": date_str,
                        "meeting_type": meeting_type, "meeting_title": clean_title,
                        "source_url": pdf_url}
        if minutes_url:
        ...
        if parsed_items:
            ...
            for pit in parsed_items:
                ...
                agenda_item_dicts.append({
                    ...
                    "agenda_item_title": pit["agenda_item_title"],
                    "agenda_item_text": "",
        ...
        supporting_doc_dicts = [
            {"agenda_item_id": "0", "agenda_item_number": "1",
             "document_title": f"Agenda: {clean_title}", "document_url": pdf_url,
             "document_type": "Agenda", "file_name": f"{body_slug}_{date_str}_agenda.pdf",
             "file_extension": ".pdf"},
        ]
        if minutes_url:
            supporting_doc_dicts.append({
                "agenda_item_id": "0", "agenda_item_number": "1",
                "document_title": f"Minutes: {clean_title}", "document_url": minutes_url,
                "document_type": "Minutes", "file_name": f"{body_slug}_{date_str}_minutes.pdf",
                "file_extension": ".pdf",
            })

        try:
            replace_meeting_data_safe(session, body_slug, meeting_id,
                                       meeting_dict, agenda_item_dicts,
                                       supporting_doc_dicts=supporting_doc_dicts)
            meetings_created.append({"body": body_slug, "meeting_id": meeting_id,
                                     "meeting_date": date_str, "title": clean_title,
                                     "agenda_url": pdf_url, "minutes_url": minutes_url})
        except Exception as e:
            print(f"  ERROR persisting meeting {meeting_id}: {e}")
            continue

        if download:
            from scraper.common.io_utils import ensure_dir
            docs_dir = _PROJECT_ROOT / "data" / "tempe-subcommittees" / body_slug / date_str
            ensure_dir(str(docs_dir))
            try:
                _helper_download(pdf_url, str(docs_dir / "agenda.pdf"))
            except Exception as e:
                print(f"    WARNING: Agenda PDF download failed: {e}")
            if minutes_url:
                try:
                    _helper_download(minutes_url, str(docs_dir / "minutes.pdf"))
                except Exception as e:
...
def sync_all(download: bool = False, limit: int | None = None) -> int:
    """Sync all Tempe council subcommittees. Returns total meetings created."""
    total = 0
