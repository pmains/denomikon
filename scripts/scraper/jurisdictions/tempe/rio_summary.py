"""Parse Tempe Rio Salado Community Facilities District Board Summary PDFs.
...
Role titles differ and are normalized before parsing.
"""
...
def rio_summary_document_names(meeting_id: int, meeting_date: str = "") -> list[str]:
    ...
    if meeting_date:
        try:
            ...
            for ts in ("6_00_00_PM", "5_45_00_PM"):
                names.append(
                    f"Rio_Salado_Community_Facilities_District_Board_Special_Meeting_"
                    f"{meeting_id}_Agenda_{date.month}_{date.day}_{date.year}_{ts}.pdf"
                )
        except ValueError:
            pass
    return names
...
def _normalize_rio_text(text: str) -> str:
    ...
    text = text.replace("Rio Salado Community Facilities District Board", "Tempe City Council")
    return text
...
def extract_rio_votes(meeting_id: int, meeting_date: str = "",
    ...
    candidates = rio_summary_document_names(meeting_id, meeting_date)
    pdf_bytes = None
    for name in candidates:
        try:
            ...
            if pdf_bytes and len(pdf_bytes) >= 1000 and not pdf_bytes[:5] == b"<!DOC":
                log.info("RIO Summary PDF found: %s (%d bytes)", name, len(pdf_bytes))
                break
            pdf_bytes = None
        except Exception:
            continue
